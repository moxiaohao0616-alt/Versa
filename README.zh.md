...
Versa 还在 1.0 之前 —— 第一次用的话,**稳定**里的功能放心用,**实验性**里的功能能用但偶尔会出"咦"。

**稳定** —— 常用路径打磨较多,可当生产可用:

- 文件级 / Hunk 级暂存 · 提交 · 推送 · 拉取 · fetch
- 分支列表 / 新建 / 切换 / 删除 / 改名 · Tag 全套 · Stash
- 历史视图(graph + 每条 commit 的 kebab 菜单)· commit 详情(文件 + diff)· 文件/代码块历史
- Diff 查看器(统一视图 + 联动并排 · 单词级高亮 · 忽略空白)
- 多标签终端(仓库根目录)· 多仓工作区(`⌘P` 快切 · 拖动重排 · `⌘1-9` 跳转)
- 全局代码搜索(`⌘⇧F`,内容 + 文件名 + 未跟踪文件都搜)
- 任意文件夹"打开 / `git init`",附带预置 `.gitignore`
- 中英双语 UI

**实验性** —— 可用,但有粗糙的地方或者刚加进来:

- 三方合并 / Rebase / Revert / Cherry-pick 冲突编辑器(常见情况 OK,奇怪的空白/编码可能翻车)
- 交互式 Rebase 拖拽(后悔了就用 Reflog 回去)
- Bisect + AI 起点推荐(启发式,要核验)
- AI 功能(commit message · 解释 · review · 冲突 · PR 草稿)—— 质量看 provider;markdown 输出还在调
- Reflog "时光机" —— 撤销可靠,复杂恢复场景尽力而为
- Changelist —— 分组提交可用;文件跨组移动的边角情况有几个
- Versa Cloud 登录 —— 客户端在,但后端还没部署
- 历史分支筛选 · 仓库拖动排序 · 提交前 AI 解释改动(这几个都是刚加的)

新用户优先用稳定那一栏。实验性里的功能本身能跑,只是"咦"的概率高一点。
